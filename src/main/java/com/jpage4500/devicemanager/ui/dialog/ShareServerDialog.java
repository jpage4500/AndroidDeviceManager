...
import com.jpage4500.devicemanager.utils.NetworkUtils;
...
public class ShareServerDialog extends JPanel {
    ...
    private void initUI() {
        ...
        mainPanel.add(copyButton, "skip 1, wrap 10px");
    ...
    private void fetchPublicIpAsync() {
        new Thread(() -> {
            try {
                String publicIp = NetworkUtils.getRequest("https://api.ipify.org");
                if (publicIp != null && !publicIp.trim().isEmpty()) {
                    cachedPublicIp = publicIp.trim();
    ...
    private static class ClientTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            ...
            switch (columnIndex) {
                case 0: return client.name != null ? client.name : "Unknown";
                case 1: return client.ipAddress;
                case 2: return new java.text.SimpleDateFormat("HH:mm:ss").format(new java.util.Date(client.connectedAtMs));
                default: return null;
            }
        }
    }
